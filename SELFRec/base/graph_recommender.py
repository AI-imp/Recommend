...
from util.evaluation import ranking_evaluation,ranking_evaluation2,ranking_evaluation3
import sys
...
class GraphRecommender(Recommender):
    ...
    def fast_evaluation(self, epoch):
        ...
        temp = []
        if self.data.test_set_group:
            for i in range(5):
                temp.append(ranking_evaluation2(self.data.test_set_group[i], rec_list, [self.max_N]))
                print("*" * 20)
                print(i)
                print(" ", temp[i][1])
                print("*" * 20)
        temp2 = []
        if self.data.test_user_cold:
            rec = defaultdict(dict)
            for user in self.data.test_user_cold:
                rec[user] = rec_list[user]
            temp2.append(ranking_evaluation3(self.data.test_set_cold, rec, [self.max_N]))
            print("*" * 20)
            print(" ", temp2[:])
            print("*" * 20)
        ...
        print('-' * 120)
        # writer.add_scalar('Recall', self.bestPerformance[1]['Recall'], epoch)
        # writer.add_scalar('NDCG', self.bestPerformance[1]['NDCG'], epoch)
        # writer.close()
        return measure
