...
class data_process:
    def __init__(self, file_path, user_file_name, item_file_name, k, history, reverse=False) -> None:
        ...
        :param user_file_name: 文件名字不包含后缀名
        :param item_file_name: 文件名字不包含后缀名
        :param history: 用户交互历史(用户训练数据集)
        """
        ...
        self.k = int(k)
        self.user_emb_path = os.path.join(self.file_path, self.user_file_name + "_emb"+".pt")
        self.item_emb_path = os.path.join(self.file_path, self.item_file_name +"_emb"+ ".pt")
