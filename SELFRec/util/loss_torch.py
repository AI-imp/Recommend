...
def bpr_loss(user_emb, pos_item_emb, neg_item_emb,tittle_emb = None):
    if tittle_emb == None:
        ...
        pos_item_emb = F.normalize(pos_item_emb)
        neg_item_emb = F.normalize(neg_item_emb)
        pos_score = torch.mul(user_emb, pos_item_emb).sum(dim=1) + torch.mul(user_emb, tittle_emb[1]).sum(dim=1)
        neg_score = torch.mul(user_emb, neg_item_emb).sum(dim=1) + torch.mul(user_emb, tittle_emb[0]).sum(dim=1)
        loss = -torch.log(10e-8 + torch.sigmoid(pos_score - neg_score))
