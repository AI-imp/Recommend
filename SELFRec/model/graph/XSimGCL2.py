...
import time
# Paper: XSimGCL2 - Towards Extremely Simple Graph Contrastive Learning for Recommendation


class XSimGCL2(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(XSimGCL2, self).__init__(conf, training_set, test_set)
        args = OptionConf(self.config['XSimGCL2'])
        self.cl_rate = float(args['-lambda'])
        self.eps = float(args['-eps'])
        self.temp = float(args['-tau'])
        self.n_layers = int(args['-n_layer'])
        self.layer_cl = int(args['-l*'])
        self.model = XSimGCL2_Encoder(self.data, self.emb_size, self.eps, self.n_layers,self.layer_cl)
...
class XSimGCL2_Encoder(nn.Module):
    def __init__(self, data, emb_size, eps, n_layers, layer_cl):
        super(XSimGCL2_Encoder, self).__init__()
        self.data = data
        self.eps = eps
